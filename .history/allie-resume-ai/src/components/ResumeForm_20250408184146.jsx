...
const ResumeForm = ({ onGenerate }) => {
  ...
  useEffect(() => {
    if (location.state?.reset === true) {
      setFormData({ name: "", phone: "", email: "", jobTitle: "" });
      setSections([]);
      setTypedText({});
      setIsTyping({});
      setTypingCompleted({});
      ...
      // Prevent re-reset on back navigation
  ...
  const handleAutoFill = async (index) => {
    ...
    try {
      const response = await axios.post(
        "https://allie-resume-backend.onrender.com/generate",
        {
          prompt,
      ...
      updatedSections[index].content = text;
      setSections(updatedSections);

      startTyping(index, text);
    } catch (err) {
      console.error("AI Autofill Error: ", err);
      alert("Something went wrong while trying to fetch AI content.");
    }
  };
